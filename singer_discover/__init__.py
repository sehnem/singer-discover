...
def main():
    ...
    select_streams = dict(
        {"message": "Select Streams", "choices": stream_choices}, **base_checkbox
    )
    ...
    for i, stream in enumerate(catalog["streams"]):
        ...
        if stream["stream"] not in selected_streams:
            ...
            selections = [field_reference[n] for n in selections if n != "Select All"]

            for breadcrumb in mdata.keys():
                if breadcrumb != ():
                    if (
                        metadata.get(mdata, breadcrumb, "inclusion") == "automatic"
                    ) or (breadcrumb in selections):
